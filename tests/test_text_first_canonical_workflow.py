...
class TextFirstWorkflowTest(unittest.TestCase):
    ...
    def test_build_text_units_overlaps_caption_with_next_chunk_detection(self) -> None:
        from video_atlas.schemas import CanonicalExecutionPlan
        from video_atlas.workflows.text_first_canonical.parsing import build_text_units
        ...
        def detect_side_effect(*, messages):
            del messages
            detect_calls.append(caption_started.is_set())
            ...
            if len(detect_calls) == 2:
                return {
                    "json": [
                        {
                            "timestamp": 70.0,
                            "boundary_rationale": "phase change",
                            "segment_title": "section two",
                            "confidence": 0.9,
                        }
                    ]
                }
        ...
        def caption_side_effect(*, messages):
            del messages
            caption_started.set()
            time.sleep(0.05)
            return {"json": {"summary": "summary", "caption": "caption"}}

        text_segmentor = MagicMock()
        text_segmentor.generate_single.side_effect = detect_side_effect
        captioner = MagicMock()
        captioner.generate_single.side_effect = caption_side_effect
        execution_plan = CanonicalExecutionPlan(
            profile_name="podcast",
            chunk_size_sec=60,
        ...
        subtitle_items = [
            ...
            {"start": 35.0, "end": 39.0, "text": "end of first section"},
            {"start": 55.0, "end": 65.0, "text": "middle bridge"},
            {"start": 85.0, "end": 100.0, "text": "ending"},
        ]
        ...
        self.assertGreaterEqual(len(detect_calls), 2)
        self.assertFalse(detect_calls[0])
        self.assertTrue(detect_calls[1])
